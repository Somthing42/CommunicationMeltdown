...
public class DisplayTimer : MonoBehaviour
{
	...
	void Start()
	{
		...
		GameManager.Instance.timerTime = GameManager.Instance.startingRoundTime;
	...
	void Update () {
		...
		if (!goTimer)
		{
			...
			{
				GameManager.Instance.timerTime += GameManager.Instance.sequenceCompleteReward;
				trackTime = 0;
				goTimer = true;
...
//	}
	...
	void OtherTimer()
	{
		...
		if (GameManager.Instance) {
			//Real timer stuff.
			GameManager.Instance.timerTime -= Time.deltaTime;//Count down.
			string minutes = ((int)GameManager.Instance.timerTime / 60).ToString ();//Minutes formatting
			string seconds = (GameManager.Instance.timerTime % 60).ToString ("f2");//seconds formatting.
			timeText.text = "Timer: " + minutes + ":" + seconds;
	...
	public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info) {
		if(stream.isWriting) {
			...
			stream.SendNext(GameManager.Instance.timerTime);
		}
		else {
